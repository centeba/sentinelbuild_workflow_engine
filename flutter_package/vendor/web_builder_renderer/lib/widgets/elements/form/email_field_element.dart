...
class EmailFieldElement extends StatelessWidget {
  final PageElement element;
  final RenderMode mode;

  const EmailFieldElement(
      {super.key, required this.element, required this.mode});

  @override
  Widget build(BuildContext context) {
    final pal = OPaletteScope.of(context);
    final label = element.config['label'] as String? ?? 'Email';
    final placeholder =
        element.config['placeholder'] as String? ?? 'email@example.com';
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(OTokens.radiusSm),
            borderSide:
                BorderSide(color: pal.primaryBlue, width: 1.5),
          ),
